...
const LaporanRekapitulasiLimbahCair: React.FC = () => {
  ...
  const calculateYearlyData = (facilityData: any[]): YearlyFacilityData[] => {
    ...
    facilityData.forEach(facilityInfo => {
      ...
      // Use the same field mapping logic as in the main processing
      ...
      // Initialize facility yearly data
      ...
      reports.forEach((report: any) => {
        ...
        yearlyFacility.detail_bulanan.push({
          periode: monthName,
          volume: volume,
          status: volume > 0 ? 'Ada Laporan' : 'Tidak Ada Laporan',
          tanggal: report.created_at || report.updated_at || ''
        });
        
        // Track highest month (only if volume > 0)
        if (volume > 0 && volume > yearlyFacility.bulan_tertinggi.volume) {
          yearlyFacility.bulan_tertinggi = {
            bulan: monthName,
            volume: volume
          };
        }
        
        // Track lowest month (only if volume > 0)
        if (volume > 0 && volume < yearlyFacility.bulan_terendah.volume) {
          yearlyFacility.bulan_terendah = {
            bulan: monthName,
            volume: volume
          };
        }
      ...
      // If no reports with volume > 0, reset lowest month
      ...
      yearlyFacility.persentase_kelengkapan = Math.round((yearlyFacility.total_laporan_tahunan / 12) * 100);
  ...
  const handleViewDetail = (record: RekapitulasiData) => {
    ...
    console.log('Navigating to:', `/dashboard/admin/manajemen/laporan-rekapitulasi/limbah-cair/detail/${record.id_user}?tahun=${currentYear}`);
    
    try {
      // Navigate to detail page with facility ID and year
      router.push(`/dashboard/admin/manajemen/laporan-rekapitulasi/limbah-cair/detail/${record.id_user}?tahun=${currentYear}`);
    } catch (error) {
      console.error('Navigation error:', error);
      message.error('Gagal membuka halaman detail');
    }
  };
  ...
  const handleViewYearlyDetail = (record: YearlyFacilityData) => {
    ...
    try {
      const detailUrl = `/dashboard/admin/manajemen/laporan-rekapitulasi/limbah-cair/detail/${record.id_user}?tahun=${currentYear}`;
