...
const getReportedMonths = useCallback((facilityData: any[]) => {
  ...
  const handleView = useCallback((record: any) => {
    ...
    // Helper: map month name to number
    ...
    // Extract periode and tahun safely (support nested structure)
    const rawPeriode = record.periode ?? record.periode_nama ?? record.item?.periode ?? record.item?.periode_nama;
    const periodeNum = monthNameToNumber(rawPeriode);

    const rawTahun = record.tahun ?? record.item?.tahun;
    const tahunNum = typeof rawTahun === 'number' ? rawTahun : parseInt(String(rawTahun ?? ''), 10);

    // Extract id safely (support nested structure)
    const rawId = record.id_laporan_lab ?? record.item?.id_laporan_lab ?? record.id ?? record.item?.id;
    const idNum = typeof rawId === 'number' ? rawId : parseInt(String(rawId ?? ''), 10);
    
    // Build query parameters for filtering
    const queryParams = new URLSearchParams();
    if (!isNaN(periodeNum as any) && periodeNum) {
      queryParams.set('periode', String(periodeNum));
    }
    if (!isNaN(tahunNum)) {
      queryParams.set('tahun', String(tahunNum));
    }
    if (!isNaN(idNum)) {
      queryParams.set('id_laporan_lab', String(idNum));
    }
